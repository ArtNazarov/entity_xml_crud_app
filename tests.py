...
def test_save_record(temp_app):
    """Test saving a record (luassg format)"""
    entity_name = 'posts'
    record_data = {
        'id': 'test-record-123',
        'title': 'Test Title',
    ...
    # Check file content (luassg format)
    tree = ET.parse(expected_path)
    root = tree.getroot()

    # Root element should be entity name with id attribute
    assert root.tag == entity_name
    assert root.get('id') == 'test-record-123'
...
def test_get_record_data(temp_app):
    """Test retrieving record data (luassg format)"""
    entity_name = 'news'
    record_id = 'test-news-456'

    # First create a record by manually writing the file (luassg format)
    entity_dir = os.path.join(temp_app.data_dir, entity_name)
    ...
    # Create XML file manually in luassg format
...
def test_delete_record(temp_app):
    ...
    # Create a record by manually writing the file (luassg format)
    entity_dir = os.path.join(temp_app.data_dir, entity_name)
    os.makedirs(entity_dir, exist_ok=True)
...
def test_file_naming_convention(temp_app):
    ...
    # Save a record using a simple direct approach (luassg format)
    entity_dir = os.path.join(temp_app.data_dir, entity_name)
    os.makedirs(entity_dir, exist_ok=True)
    ...
    # Verify luassg format
    tree = ET.parse(filepath)
    root = tree.getroot()
    assert root.tag == entity_name
    assert root.get('id') == record_id
...
def test_entity_directory_structure(temp_app):
    ...
    # Create a test file (luassg format)
    filename = f"{entity_name}-{record_id}.xml"
    filepath = os.path.join(entity_dir, filename)
...
def test_save_record_integration(temp_app):
    """Integration test for save_record method (luassg format)"""
...
# Test loading existing luassg format files
def test_load_luassg_format(temp_app):
    """Test loading records in luassg format"""
    entity_name = 'gallery'
    record_id = 'test-gallery-001'

    # Add entity structure
    temp_app.entities[entity_name] = {
        'fields': [
            {'name': 'image_title', 'type': 'oneline'},
            {'name': 'image_description', 'type': 'multiline'},
            {'name': 'image_alt_text', 'type': 'oneline'},
            {'name': 'image_src', 'type': 'oneline'}
        ],
        'records': {}
    }

    # Create a record in luassg format manually
    entity_dir = os.path.join(temp_app.data_dir, entity_name)
    os.makedirs(entity_dir, exist_ok=True)
    ...
    # Create XML file in luassg format (ArtNazarov/luassg format)
    root = ET.Element(entity_name)
    root.set('id', record_id)
    ET.SubElement(root, 'image_title').text = 'The kitten'
    ET.SubElement(root, 'image_description').text = 'Some description'
    ET.SubElement(root, 'image_alt_text').text = 'Some alt text'
    ET.SubElement(root, 'image_src').text = 'https://upload.wikimedia.org/wikipedia/commons/9/9b/Photo_of_a_kitten.jpg'

    tree = ET.ElementTree(root)
    tree.write(filepath, encoding='utf-8', xml_declaration=True)

    # Load the data
    temp_app.load_entity_data_from_files(entity_name)

    # Check that data was loaded correctly
    assert 'records' in temp_app.entities[entity_name]
    assert record_id in temp_app.entities[entity_name]['records']

    record_data = temp_app.entities[entity_name]['records'][record_id]
    assert record_data['id'] == record_id
    assert record_data['image_title'] == 'The kitten'
    assert record_data['image_description'] == 'Some description'
    assert record_data['image_alt_text'] == 'Some alt text'
    assert record_data['image_src'] == 'https://upload.wikimedia.org/wikipedia/commons/9/9b/Photo_of_a_kitten.jpg'
